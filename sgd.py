...
class LogReg:
    ...
    def sg_update(self, train_example):
        ...
        old_beta = self.beta
        x_i,y_i = train_example.x, train_example.y

        BX = np.dot(old_beta, x_i)
        pi_i = sigmoid(BX)

        self.beta = old_beta + self.learning_rate*(y_i - pi_i)*x_i

        return self.beta

    # A function to find the top features.
    def inspect(self, vocab, limit=10):
        return torch.topk(torch.Tensor(lr.beta),10)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--step", help="Initial SG step size", type=float, default=0.1, required=False)
    argparser.add_argument("--vocab", help="Vocabulary of all features", type=str, default="data/small_guess.vocab")
    argparser.add_argument("--train", help="Training set", type=str, default="data/small_guess.buzztrain.jsonl", required=False)
    argparser.add_argument("--test", help="Test set", type=str, default="data/small_guess.buzzdev.jsonl", required=False)
    argparser.add_argument("--passes", help="Number of passes through train", type=int, default=1, required=False)

    args = argparser.parse_args()

    # with open(args.vocab, 'r') as infile:
    with open(args.vocab, mode='r', encoding='utf-8') as infile:
